...
B = {
    "name"       : "Boots Runner",
    "tagline"    : "Run Well. Live Well.",
    "emoji"      : "💊",
    "page_title" : "Boots Runner · London",
    "bg"         : "#010610",
    "surface"    : "#060E1F",
    "surface2"   : "#0A1530",
    "border"     : "#0E2040",
    ...
    "z3"         : "#78BE20",
    "green"      : "#22C55E",
    "reward"     : "Boots Advantage Points",
    "r_icon"     : "💙",
    "r_rate"     : 15,
    "unlock_at"  : 75,
    "unlock_msg" : "500 Advantage Card points loaded! Use in-store or online.",
    "cta"        : "Shop recovery essentials",
    "cta_url"    : "https://www.boots.com",
    "powered"    : "Powered by Boots",
    "brand_tip"  : "Your health. Our priority.",
    # Boots-specific health supplements per distance
    "supplements": [
        (0, 3,   "Electrolyte tablets + Vitamin C"),
        (3, 8,   "Magnesium supplement + Protein shake"),
        (8, 15,  "Recovery foam roll + Omega-3 capsules"),
        (15,100, "Sports massage gel + Iron supplement"),
    ]
}
...
.brand-tag{{font-size:12px;color:{B['accent']};margin-top:2px;letter-spacing:.5px;}}
...
.reward-card{{background:linear-gradient(135deg,{B['primary']}33,{B['accent']}11);
  border:1px solid {B['accent']}33;border-radius:16px;
  padding:20px;text-align:center;margin-bottom:12px;}}
.reward-pts{{font-size:48px;font-weight:900;color:{B['accent']};}}
.badge{{display:inline-flex;align-items:center;gap:6px;
  background:{B['primary']}33;border:1px solid {B['accent']}22;
...
.health-card{{background:{B['surface2']};border:1px solid {B['accent']}22;
...
.stButton>button:hover{{background:{B['accent']};color:{B['bg']};}}
...
def get_tfl_routes(flat,flon,tlat,tlon):
    try:
        ...
        routes=[]; labels=["Healthiest Route","Fast Route","Scenic Route"]
        ...
        return [
            {"id":0,"label":"Healthiest Route","duration_min":round(d*12),"distance_km":round(d,2),"lights":3,"efficiency":85},
...
def ask_gemini(prompt):
    try:
        ...
        return "Well done! Consistent Zone 2–3 training reduces injury risk and builds your aerobic capacity. Your body will thank you."
...
def get_supplement_rec(km):
    for lo,hi,rec in B["supplements"]:
        if lo<=km<hi: return rec
    return B["supplements"][-1][2]
...
def page_home():
    ...
    st.markdown(f"""<div class="brand-header">
      ...
      <div style="margin-left:auto;text-align:right">
        ...
        <div style="font-size:11px;color:{B['accent']}">{w['desc']}</div>
    ...
    # Health score (zone eff avg this week)
    health_score=round(sum(r["efficiency"] for r in week_runs)/len(week_runs)) if week_runs else 0
    st.markdown(f"""<div class="metric-row">
      ...
      <div class="m-card"><div class="m-val" style="color:{B['accent']}">{health_score}%</div>
        <div class="m-lbl">HEALTH SCORE</div></div></div>""",unsafe_allow_html=True)
    ...
    if ss.runs:
        ...
        with c2:
            ...
            <div style="margin-top:8px"><div style="font-size:11px;color:{B['text_dim']}">Best zone eff</div>
              <div style="font-size:26px;font-weight:800;color:{B['accent']}">{best}%</div></div>
            <div style="margin-top:8px"><div style="font-size:11px;color:{B['text_dim']}">Advantage pts</div>
              <div style="font-size:26px;font-weight:800;color:{B['primary']}">{ss.total_pts}</div></div></div>""",unsafe_allow_html=True)
    else:
        st.markdown(f"""<div class="card" style="text-align:center;padding:32px">
        <div style="font-size:48px;margin-bottom:12px">💙</div>
        <div style="font-size:18px;font-weight:700">Your health journey starts here</div>
        <div style="font-size:14px;color:{B['text_dim']};margin-top:6px">Earn Boots Advantage points on every Zone km</div>
        </div>""",unsafe_allow_html=True)
    ...
    st.markdown(f"""<div class="card">
      ...
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin:8px 0">
        ...
        <div style="font-size:12px;color:{B['text_dim']}">{pts_to} pts to 500 Advantage</div>
      </div>
...
def page_route():
    ...
    if ss.start_pin: folium.Marker(ss.start_pin,icon=folium.Icon(color="blue",icon="play"),popup="Start").add_to(m)
    ...
    if ss.start_pin and ss.end_pin:
        ...
        if not ss.routes or st.button("🔍 Find Healthiest Routes",use_container_width=True):
            with st.spinner("Calculating healthiest routes..."):
        ...
        if ss.routes:
            ...
            for i,rt in enumerate(ss.routes):
                ...
                bdr=f"border:2px solid {B['accent']};" if sel else ""
                stars="⭐⭐⭐" if rt["efficiency"]>=80 else "⭐⭐" if rt["efficiency"]>=60 else "⭐"
                kcal=round(rt["distance_km"]*65)
                st.markdown(f"""<div class="card" style="{bdr}">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start">
                    <div><div style="font-size:15px;font-weight:700">{rt['label']}</div>
                      <div style="font-size:13px;color:{B['text_dim']};margin-top:4px">{rt['distance_km']} km · {rt['duration_min']} min · ~{kcal} kcal</div></div>
            ...
            if ss.chosen_route is not None:
                ...
                supp=get_supplement_rec(rt["distance_km"])
                ...
                st.markdown(f"""<div class="health-card">
                  <div class="card-label">💊 RECOMMENDED RECOVERY</div>
                  <div style="font-size:14px;font-weight:700;margin-top:6px">{supp}</div>
                  <div style="font-size:12px;color:{B['text_dim']};margin-top:2px">Available at Boots stores nationwide</div>
                </div>""",unsafe_allow_html=True)
...
def page_running():
    ...
    st.markdown(f"<div style='font-size:13px;color:{B['accent']};text-align:center;letter-spacing:2px;margin-bottom:8px'>● LIVE RUN · BOOTS HEALTH TRACKING</div>",unsafe_allow_html=True)

    progress=min(100,ss.current_dist/tgt_km*100) if tgt_km>0 else 0
    kcal_so_far=round(ss.current_dist*65)
    st.markdown(f"""<div class="card" style="text-align:center;padding:24px">
      ...
      <div style="font-size:13px;color:{B['text_dim']};margin-top:6px">of {tgt_km} km · ~{kcal_so_far} kcal burned</div>
    ...
    with c1:
        ...
        <div class="ring-container">{svg_ring(zone_pct,B['z3'] if zone_pct>=60 else B['z2'],label="health eff")}</div>
        </div>""",unsafe_allow_html=True)
    with c2:
    ...
    zn={1:"Zone 1 – Easy",2:"Zone 2 – Fat Burn ✓",3:"Zone 3 – Cardio ✓",4:"Zone 4 – Hard ↑",5:"Zone 5 – Max ⚠️"}
    ...
    if zone not in [2,3] and ss.total_time>0:
        adv="slow down to reduce cardiac stress" if zone>3 else "pick up the pace for better health benefit"
        st.markdown(f"""<script>
        var u=new SpeechSynthesisUtterance("Health check: Zone {zone}. {adv}.");
...
def page_summary():
    ...
    st.markdown(f"""<div class="card" style="text-align:center">
      <div class="card-label">ZONE 2~3 HEALTH EFFICIENCY</div>
      <div class="ring-container">{svg_ring(eff,ec,size=160,label="health eff")}</div>
      <div style="font-size:14px;color:{B['text_dim']};margin-top:6px">{stars} Marathon Health Score · ~{kcal} kcal</div>
    </div>""",unsafe_allow_html=True)
    ...
    st.markdown(f"""<div class="metric-row">
      ...
      <div class="m-card"><div class="m-val" style="color:{B['accent']}">{eff}%</div><div class="m-lbl">EFF</div></div>
    </div>""",unsafe_allow_html=True)

    pts=last["pts_earned"]
    st.markdown(f"""<div class="reward-card">
      <div style="font-size:13px;color:{B['text_dim']}">Advantage Card points earned</div>
      <div class="reward-pts">+{pts}</div>
      <div style="font-size:14px;color:{B['text_dim']}">{B['reward']} · Total: {ss.total_pts}</div>
    </div>""",unsafe_allow_html=True)

    # Boots-specific: health supplement recommendation
    supp=get_supplement_rec(last['distance_km'])
    st.markdown(f"""<div class="health-card">
      <div class="card-label">💊 BOOTS RECOVERY RECOMMENDATION</div>
      <div style="font-size:15px;font-weight:700;margin-top:8px">{supp}</div>
      <div style="font-size:13px;color:{B['text_dim']};margin-top:4px">
        {last['distance_km']} km run · ~{kcal} kcal burned · {B['brand_tip']}</div>
      <a href="{B['cta_url']}" target="_blank"
         style="display:block;margin-top:10px;color:{B['accent']};font-size:14px;font-weight:700">{B['cta']} →</a>
    </div>""",unsafe_allow_html=True)

    if ss.total_pts>=B["unlock_at"]:
        st.markdown(f"""<div class="card" style="text-align:center;
            background:linear-gradient(135deg,{B['primary']}33,{B['accent']}11)">
    ...
    if st.button("🤖 AI Coach Feedback",use_container_width=True):
        with st.spinner("Analysing your health data..."):
            fb=ask_gemini(f"London marathon running coach with health focus. 3-sentence feedback on health benefits. "
                         f"Run: {last['distance_km']}km, {last['duration_min']}min, "
                         f"Zone 2-3 eff {eff}%, {last['lights']} traffic lights, avg HR {last['avg_hr']}bpm, ~{kcal} kcal.")
        st.markdown(f"""<div class="card"><div class="card-label">AI HEALTH COACH</div>
...
# ── HISTORY ───────────────────────────────────────────────────────────────────
def page_history():
    st.markdown(f"<div style='font-size:22px;font-weight:800;margin:16px 0 12px'>📊 Health & Performance</div>",unsafe_allow_html=True)
    if not ss.runs:
        st.markdown(f"""<div class="card" style="text-align:center;padding:40px">
        <div style="font-size:48px;margin-bottom:12px">📈</div>
    ...
    st.markdown(f"""<div class="metric-row">
      ...
      <div class="m-card"><div class="m-val">{tkcal}</div><div class="m-lbl">TOTAL KCAL</div></div>
      <div class="m-card"><div class="m-val" style="color:{B['accent']}">{ae}%</div><div class="m-lbl">AVG HEALTH</div></div>
    </div>""",unsafe_allow_html=True)

    if len(ss.runs)>=2:
        df=pd.DataFrame([{"Run":f"#{i+1}","Zone Efficiency (%)":r["efficiency"],"Traffic Lights":r["lights"]}
                         for i,r in enumerate(ss.runs)])
        st.markdown(f"<div class='card-label' style='margin:8px 0 4px'>HEALTH SCORE TREND</div>",unsafe_allow_html=True)
        st.line_chart(df.set_index("Run")[["Zone Efficiency (%)"]],color=[B["accent"]])
    ...
    for r in reversed(ss.runs):
        ...
        st.markdown(f"""<div class="run-card">
          ...
          <div style="text-align:right">
            ...
            <div style="font-size:11px;color:{B['text_dim']}">Health eff</div>
            <div style="font-size:14px;font-weight:700;color:{B['primary']};margin-top:6px">+{r['pts_earned']} pts</div>
          </div></div>""",unsafe_allow_html=True)
    ...
    <div class="metric-row">
      ...
      <div class="m-card"><div class="m-val">{pb_e}%</div><div class="m-lbl">HEALTH PB</div></div>
...
def page_profile():
    ...
    st.markdown(f"""<div style="text-align:center;padding:16px 0">
      ...
      <div style="font-size:13px;color:{B['text_dim']}">Zone 2~3 Runner · Boots Advantage</div>
    </div>""",unsafe_allow_html=True)
    ...
    zones_data=[("Zone 1 – Easy","#94A3B8",ss.max_hr*.5,ss.max_hr*.6),
                ("Zone 2 – Fat Burn ✓",B["z2"],ss.max_hr*.6,ss.max_hr*.7),
                ("Zone 3 – Cardio ✓",B["z3"],ss.max_hr*.7,ss.max_hr*.8),
