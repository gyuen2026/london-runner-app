...
# ==================================================
    ...
    # Generate 5 Distinct Routes based on velocity sync models
    ROUTES_5 = [
        ...
        for r in ROUTES_5:
            with st.container():
                ...
                <div style="padding:12px; border-left:4px solid {r['color']}; background:#1C1C1E; margin-bottom:8px;">
                    ...
                    <div style="font-size:12px; color:#AEED2; margin-top:2px;">{r['desc']} · <b>{r['signals']} Stops</b></div>
                </div>
                """, unsafe_allow_html=True)
...
# ==================================================
    ...
    # Fallback to predefined routes to get rendering path coordinates
    sc = st.session_state.start_coord
    ec = st.session_state.end_coord
    path_nodes = [sc, (sc[0]+0.001, sc[1]-0.002), (ec[0]-0.001, ec[1]+0.002), ec]
    
    # Calculate current tracking markers
    total_distance = haversine_distance(sc, ec)
    if st.session_state.km_completed == 0.0:
        st.session_state.current_coord = sc
    
    # Generate contextual turn-by-turn direction banner based on progress
    ...
    with c2:
        # Emulating live pace adaptation
    ...
    with col_map:
        ...
        # Draw target route line
        folium.PolyLine(path_nodes, color="#E5FF00", weight=6, opacity=0.8).add_to(m_live)
        # Draw user's current moving position anchor
        folium.Marker(list(st.session_state.current_coord), icon=folium.Icon(color="blue", icon="dot", prefix="fa"), popup="Your Position").add_to(m_live)
    ...
    with col_ctrl:
        ...
        # Simulation step button to increment location data points
